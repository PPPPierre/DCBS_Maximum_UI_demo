...
class Panel_ChargeStopped(wx.Panel):
    ...
    def UI_init(self):
        MyWidget.Background(self, "Maximus Demo Interface - Background Only-05.jpg")
        ...
        box2.AddMany([(box1, 0, wx.ALIGN_CENTRE | wx.TOP, 377),
                      (self.buttonMain, 0, wx.ALIGN_CENTRE | wx.TOP, 130)])
        self.SetSizer(box2)

    def activate(self):
        print("----------Charge Stopped Page-----------")
        self.chart_generate()
    ...
    def chart_generate(self):
        ...
        self.data.plot_measurement(title=title, type='v+i', mode='save', path="./TestGraph/temp_graph.png")
    ...
    def save_data_helper(self):
        path = self.data.save_data('v+i', self.get_test_title())
        if path != '0':
            title = self.get_test_title()
            file_path = path + '_figure.png'
            self.data.plot_measurement(title, 'v+i', mode='save', path=file_path)
    ...
    def get_test_title(self):
        title = 'Charge ' + \
                'Target voltage ' + str(self.data.targetVoltage) + ' V ' + \
                'Max current ' + str(self.data.maxCurrent) + ' A ' + \
                'Time ' + str(self.data.testTime // 60) + ' min ' + \
                'End current ' + str(self.data.endCurrent) + ' A'
        return title
