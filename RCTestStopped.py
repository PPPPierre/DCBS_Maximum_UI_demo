...
class Panel_RCTestStopped(wx.Panel):
    ...
    def UI_init(self):
        MyWidget.Background(self, "Maximus Demo Interface - Background Only - RC Test-12.jpg")
    ...
    def chart_generate(self):
        ...
        self.data.plot_measurement(title=title, type='v+c', mode='save', path="./TestGraph/temp_graph.png")
    ...
    def save_data_helper(self):
        path = self.data.save_data('v+c', self.get_test_title())
        if path != '0':
            title = self.get_test_title()
            file_path = path + '_figure.png'
            self.data.plot_measurement(title, 'v+c', mode='save', path=file_path)
    ...
    def get_test_title(self):
        title = 'RC ' + \
                'Current ' + str(self.data.RC_current) + ' A ' + \
                'Time ' + str(self.data.RC_second // 60) + ' min'
        return title
